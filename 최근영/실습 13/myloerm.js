...
// 받아온 버튼을 클릭했을 경우 이벤트 생성
checkBtn.addEventListener("click", function () {
    // 중복값을 체크해준다.
    let check_value = [];
    // 정해진 리스트의 값을 넘었을 경우
    if (textBox.value > answer_list.length) {
        alert(`값은 ${answer_list.length}을 초과할 수 없습니다.`)
    }
    // 입력된 값이 리스트 값을 넘지 않았을 경우
    else {
        // 문장을 넣을 박스의 자식 요소가 존재할 경우 => 박스에 내용이 존재할 경우
        if (textBox.childElementCount) {
            // allRemoveChild 라는 함수를 통해서 내부의 자식요소를 전부 지워준다.
            allRemoveChild(textBox);
            for (i = 0; i < textCnt.value; i++) {
                // 랜덤 함수를 통해서 범위를 answer_list 값까지 제한하여 정수값을 출력한다.
                let ans_num = Math.floor(Math.random() * answer_list.length);
                // 나온 값이 check_value 라는 list를 통하여 중복되는 값인지 판단하는 로직
                if (check_value.includes(ans_num)) {
                    while (true) {
                        ans_num = Math.floor(Math.random() * answer_list.length);
                        if (check_value.includes(ans_num)) {
                            continue;
                        }
                        else {
                            check_value.push(ans_num);
                            break;
                        }
                    }
                }
                ...
                // 제목과 내용의 형태로 값을 출력한다.
                p.innerText = `${answer_list[ans_num].title} : ${answer_list[ans_num].context}`;
                textBox.appendChild(p);
            }
        }
        // 문장을 넣을 박스의 자식요소가 없을 경우 반복문을 통해서 출력해준다.
        else {
            for (i = 0; i < textCnt.value; i++) {
                // 랜덤 함수를 통해서 범위를 answer_list 값까지 제한하여 정수값을 출력한다.
                let ans_num = Math.floor(Math.random() * answer_list.length);
...
// 부모의 자식이 존재할 경우 반복문을 통하여 모든 자식을 지워주는 함수
function allRemoveChild(parent) {
    // 매개변수로 받아온 변수의 자식요소가 없을때까지 값을 지워준다.
